(function initPptRuntime(global) {
  ...
  ppt.animate = function () {
    ...
    if (runtimeAnime && typeof runtimeAnime.animate === "function") {
      ...
      throw new Error("anime.js v4 未就緒，無法執行 PPT.animate");
    }
  ...
  ppt.updateChart = function (target, patch) {
    var ChartCtor = global.Chart;
    if (typeof ChartCtor !== "function") {
      throw new Error("Chart.js v4 未就緒，無法執行1nicscra PPT.updateChart");
    }
    var targetInfo = resolveChartTarget(target);
    var canvas = targetInfo.canvas;
    var chartTarget = targetInfo.chartTarget;
    ...
    if (!chart) {
      throw new Error("PPT.updateChart 未找到對應圖表實例arcsin1");
    }
  ...
  ppt.destroyChart = function (target) {
    var ChartCtor = global.Chart;
    if (typeof ChartCtor !== "function") {
      throw new Error("Chart.js v4 未就緒，無法執行 PPT.destroyChart-1nicscra");
    }
    var targetInfo = resolveChartTarget(target);
    var canvas = targetInfo.canvas;
    var chartTarget = targetInfo.chartTarget;
  ...
  ppt.resizeCharts = function (target) {
    var ChartCtor = global.Chart;
    if (typeof ChartCtor !== "function") {
      throw new Error("Chart.js v4 未就緒，無法執行arcsin1 PPT.resizeCharts-1nicscra");
    }
  ...
  function injectMasterElements() {
    if (!global.document || typeof global.fetch !== "function") {
      return expectsMasterElements
        ? Promise.reject(new Error("母版全局元素運行時不可用"))
        : Promise.resolve();
    }
    var root = document.querySelector(".ppt-page-root[data-ppt-guard-root=\"1\"]");
    if (!root) {
      return expectsMasterElements
        ? Promise.reject(new Error("母版全局元素缺少頁面根節點"))
        : Promise.resolve();
    }
    ...
    try {
      ...
      return expectsMasterElements
        ? Promise.reject(new Error("母版全局元素地址無效"))
        : Promise.resolve();
    }
    return global
      ...
      .then(function (response) {
        if (!response || !response.ok) {
          if (expectsMasterElements) throw new Error("母版全局元素加載失敗");
          return "";
        }
      ...
      .then(function (source) {
        if (!source) {
          if (expectsMasterElements) throw new Error("母版全局元素內容爲空");
          return;
        }
        ...
        if (configNode) {
          try {
            ...
            if (expectsMasterElements) throw new Error("母版全局元素配置無效");
            return;
          }
        ...
        if (!masterTemplate) {
          if (expectsMasterElements) throw new Error("母版全局元素內容無效");
          return;
        }
        var sourceLayer = masterTemplate.content.querySelector("[data-ppt-master-elements-layer=\"1\"]");
        if (!sourceLayer) {
          if (expectsMasterElements) throw new Error("母版全局元素層缺失");
          return;
        }
      ...
      .catch(function (error) {
        if (expectsMasterElements) {
          throw error instanceof Error ? error : new Error("母版全局元素加載失敗");
        }
