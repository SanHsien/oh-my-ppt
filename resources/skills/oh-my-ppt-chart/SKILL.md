...
Do not mix units or meanings in a single Chart.js dataset or value axis. A bar chart whose x-axis is "變化率 %" must contain only percentages; do not put headcount values such as `850` or `1400` into that same dataset. If the source has both absolute counts and percent changes, choose one expression:
...
Chart.js category labels are plain text. Do not put HTML such as `<br>`, `<span>`, or inline styles inside `data.labels`; Chart.js will draw that HTML literally or mis-measure it. For multi-line category labels, use string-array labels (e.g. `['AI調校師', '約80→1,400']`) or keep details in tooltip callbacks / nearby annotations.
...
data: { labels: ['AI敘事設計師', 'AI調校師', '原畫師'], datasets: [{ label: '變化率', data: [850, 1650, 17.9] }] }

<!-- Chart.js labels are not HTML -->
labels: ['AI調校師<br><span>約80→1,400</span>']
```
