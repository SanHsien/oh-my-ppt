...
export class GuardedFilesystemBackend extends FilesystemBackend {
  constructor(
    ...
    this.editBlockedReason =
      ...
      '當前任務禁止調用 edit_file。請使用 update_single_page_file(pageId, content) 或 update_page_file(pageId, content)。'
    this.writeBlockedReason =
      options.writeBlockedReason || '當前任務禁止調用 write_file。請使用受控的頁面寫入工具。'
  }
  ...
  async edit(
    ...
    if (this.allowedEditPaths && !this.allowedEditPaths.has(normalizeVirtualPath(filePath))) {
      ...
      return {
        error:
          `當前精修任務只能使用 edit_file 修改目標頁面${allowedPaths ? `：${allowedPaths}` : ''}。`
      }
    }
