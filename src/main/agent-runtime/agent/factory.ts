...
export function createSessionEditAgent(args: CreateSessionEditAgentArgs): DeepAgentStreamResult {
  ...
  const backend = new GuardedFilesystemBackend({
    ...
    editBlockedReason: disableNativeEditFile
      ? '當前編輯任務禁止使用 edit_file。請改用 update_single_page_file(pageId, content) 或 update_page_file(pageId, content)。'
      : undefined,
    writeBlockedReason:
      '當前編輯任務禁止使用 write_file。請使用 update_single_page_file(pageId, content)、update_page_file(pageId, content) 或允許的 edit_file。'
  })
...
export function createSessionDeckAgent(args: CreateSessionDeckAgentArgs): DeepAgentStreamResult {
  ...
  const backend = new GuardedFilesystemBackend({
    ...
    editBlockedReason: context.templatePageReadRequired
      ? '當前模板生成任務禁止使用 edit_file。請使用 update_template_page_file(pageId, content)。'
      : '當前生成/全局編輯任務禁止使用 edit_file。請使用 update_single_page_file(pageId, content) 或 update_page_file(pageId, content)。'
  })
  const requiredSkillNames = getRequiredProductSkillNamesForSlideSize(context.slideSize)
