...
export function buildSinglePageGenerationPrompt(args: {
  ...
  const shouldMentionWriteToolFix =
    /頁面未寫入|沒有成功調用|not written|update_single_page_file|update_template_page_file|佔位|placeholder/i.test(
      previousError
    )
  const shouldMentionTemplateSkeletonFix =
    /模板骨架|skeleton|background\/decorative|背景\/裝飾資源|CSS url|SVG image|local asset/i.test(
      previousError
    )
  ...
  const sectionAgendaInstructions = isSectionAgendaPage
    ? referenceTextLocked
      ? [
          ...
          '- Do not add fixed visible labels such as “Agenda”, “目錄”, “本章包括”, or “章節導航” unless the source itself contains them.'
        ]
      : [
          '',
          'Section agenda page requirements:',
