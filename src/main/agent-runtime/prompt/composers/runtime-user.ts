...
const hasSourceMaterialCue = (value: string): boolean =>
  /參考文檔|源文檔|參考資料|源資料|解析.*大綱|建議大綱|每頁要點|必須保留|reference[-\s]?document|source[-\s]?document|source[-\s]?material|recommended outline|per-page points|facts\/metrics/i.test(
    value
  );
...
export function buildPlanningUserPrompt(args: {
  ...
  const hasExplicitPageHint = /第\s*\d+\s*頁|(?:page|slide)\s*\d+/i.test(args.userMessage);
