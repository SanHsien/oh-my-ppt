...
const parseCredentials = (
  ...
  if (!accessKeyId || !secretAccessKey) {
    throw new Error('即夢 3.0 需要 Access Key ID 和 Secret Key。')
  }
...
export const jimengAdapter: ImageGenerationProviderAdapter = {
  getDefaultSize(config) {
    return resolveConfiguredDefaultImageSize(config) || '1664x936'
  },
  ...
  async generate(config, input) {
    ...
    try {
      for (let index = 0; index < input.count; index += 1) {
        ...
        if (!taskId) throw new Error('即夢 3.0 未返回 task_id')
        ...
        for (let poll = 0; poll < maxPolls; poll += 1) {
          ...
          if (status === 'done') {
            ...
            if (images.length === 0) throw new Error('即夢 3.0 未返回圖片')
            results.push(...images)
          ...
          if (status === 'not_found' || status === 'expired') {
            throw new Error(`即夢 3.0 任務狀態異常：${status}`)
          }
        }
