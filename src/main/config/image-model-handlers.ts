...
const IMAGE_MODEL_VERIFY_PROMPT = '生成一隻卡通貓'
...
export function registerImageModelHandlers(ctx: IpcContext): void {
  ...
  ipcMain.handle('imageModels:upsert', async (_event, payload) => {
    ...
    if (!name) throw new Error(uiText(locale, '請填寫生圖模型名稱。', 'Enter image model name.'))
    if (modelConfig === '{}') {
      throw new Error(uiText(locale, '請填寫生圖模型配置。', 'Enter image model config.'))
    }
  ...
  ipcMain.handle('imageModels:setActive', async (_event, id) => {
    ...
    if (typeof id !== 'string' || id.trim().length === 0) {
      throw new Error(uiText(locale, '生圖模型配置 ID 不能爲空。', 'Image model config ID is required.'))
    }
    try {
      await db.setActiveImageModelConfig(id.trim())
    } catch (error) {
      if (error instanceof Error && error.message === 'Image model config does not exist') {
        throw new Error(uiText(locale, '生圖模型配置不存在。', 'Image model config does not exist.'))
      }
      throw error
    }
    return { success: true }
  ...
  ipcMain.handle('imageModels:verify', async (_event, payload) => {
    ...
    if (modelConfig === '{}') {
      return {
        valid: false,
        message: uiText(locale, '請先填寫生圖模型配置。', 'Enter image model config first.')
    ...
    try {
      ...
      return {
        valid: true,
        message: uiText(locale, '已成功生成測試圖片。', 'A test image was generated successfully.'),
