...
const normalizeVerifyErrorMessage = (
  ...
  if (options.provider === 'openai' && isThinkingParameterError) {
    return uiText(
      options.locale,
      '當前模型不支持 thinking 參數，請在模型設置中改爲“不發送 thinking 參數”。',
...
export function registerSettingsHandlers(ctx: IpcContext): void {
      ...
      try {
        ...
        throw new Error(
          uiText(
            ...
            `代理設置無效：${proxyError instanceof Error ? proxyError.message : '請檢查地址格式'}`,
  ...
  ipcMain.handle('settings:upsertModelConfig', async (_event, payload) => {
    ...
    if (!name) throw new Error(uiText(locale, '請填寫模型名稱。', 'Enter model name.'))
    if (!model) throw new Error(uiText(locale, '請填寫 model。', 'Enter model.'))
    if (!apiKey) throw new Error(uiText(locale, '請填寫 api_key。', 'Enter api_key.'))
  ...
  ipcMain.handle('settings:setActiveModelConfig', async (_event, id) => {
    ...
    if (typeof id !== 'string' || id.trim().length === 0) {
      throw new Error(uiText(locale, '模型配置 ID 不能爲空。', 'Model config ID is required.'))
    }
  ...
  ipcMain.handle(
    ...
    async (
      ...
      if (typeof apiKey !== 'string' || apiKey.trim().length === 0) {
        return {
          valid: false,
          message: uiText(locale, '請先填寫 api_key。', 'Enter api_key first.')
        }
      }
      if (typeof model !== 'string' || model.trim().length === 0) {
        return { valid: false, message: uiText(locale, '請先填寫 model。', 'Enter model first.') }
      }
      ...
      try {
        ...
        return { valid: true, message: uiText(locale, '連接驗證成功。', 'Connection verified.') }
      } catch (error) {
        const message =
          normalizeVerifyErrorMessage(error, { locale, provider }) ||
          uiText(
                locale,
                '連接驗證失敗，請檢查 api_key、model 或 base_url。',
