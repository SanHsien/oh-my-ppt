  ...
  for (const row of sessions.rows || []) {
    ...
    generatedPages.forEach((page, index) => {
      ...
      pageMap.set(pageId, {
        pageId,
        pageNumber,
        title: String(page.title || `第 ${pageNumber} 頁`),
    ...
    failedPages.forEach((page, index) => {
      ...
      pageMap.set(pageId, {
        ...
        htmlPath: resolveLegacyPagePath(page, projectDir, pageId),
        status: 'failed',
        error: String(page.reason || page.error || '舊 metadata 記錄的失敗頁'),
        retryCount: toPositiveInt(page.retryCount ?? page.retry_count, 0)
      })
    })
...
const patchSessionPagesFromLegacy = async (args: {
  ...
  for (const row of sessions.rows || []) {
    ...
    for (const failed of failedPages) {
      ...
      failedById.set(fileSlug, String(failed.reason || failed.error || '頁面生成失敗'))
      pageMap.set(fileSlug, {
        pageNumber,
        fileSlug,
        title: String(failed.title || `第 ${pageNumber} 頁`),
        htmlPath: '',
        status: 'failed',
        error: String(failed.reason || failed.error || '頁面生成失敗')
      })
    }
    ...
    for (let index = 0; index < generatedPages.length; index += 1) {
      ...
      const title = String(page.title || `第 ${pageNumber} 頁`)
      ...
      const status = failedReason
        ...
        : fsExistsSafe(htmlPath)
          ? ('completed' as const)
          : ('failed' as const)
      const error = failedReason || (status === 'failed' ? '頁面文件不存在' : null)
      pageMap.set(fileSlug, {
        pageNumber,
        fileSlug,
        title,
    ...
    for (let index = 0; index < pages.length; index += 1) {
        ...
        status === 'failed'
          ? String(
              getRowValue(row, 'error') ||
                (fsExistsSafe(htmlPath) ? '頁面生成失敗' : '頁面文件不存在')
            )
          : null
      ...
      await db
        ...
        .values({
          ...
          fileSlug,
          pageNumber,
          title: String(getRowValue(row, 'title') || `第 ${pageNumber} 頁`),
          htmlPath,
          status,
          error,
