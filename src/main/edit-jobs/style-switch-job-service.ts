...
const errorMessage = (error: unknown): string =>
  error instanceof Error && error.message ? error.message : String(error || '風格切換失敗')

class StyleSwitchCommittedPageFinalizationError extends Error {
  constructor(cause: unknown) {
    super(`頁面歷史已寫入，但頁面狀態更新失敗：${errorMessage(cause)}`)
...
class StyleSwitchHistoryCommitError extends Error {
  constructor(cause: unknown) {
    super(`頁面版本歷史寫入失敗：${errorMessage(cause)}`)
...
const toRelativeProjectPath = (projectDir: string, filePath: string): string => {
  ...
  if (!relative || relative.startsWith('../') || path.isAbsolute(relative)) {
    throw new Error(`風格切換頁面路徑不在項目目錄內：${filePath}`)
  }
  return relative
}
...
export class StyleSwitchJobService {
  ...
  async start(
    ...
    if (!styleId) throw new Error('styleId 不能爲空')
    ...
    try {
      ...
      if (!style || style.active === false) throw new Error('選擇的風格不存在或已停用')
      ...
      if (requestedPageIds.length > 0 && selectedPages.length !== requestedPageIds.length) {
        throw new Error('重試頁面不存在或已被刪除')
      }
      if (selectedPages.length === 0) throw new Error('沒有可切換風格的頁面')
      ...
      if (context.runId !== lease.jobId) {
        throw new Error('風格切換 runId 與 JobCoordinator lease 不一致')
      }
      ...
      this.ctx.emitGenerateChunk(sessionId, {
        ...
        payload: {
          runId: context.runId,
          stage: 'style-switch',
          label: context.appLocale === 'en' ? 'Preparing style switch' : '正在準備切換風格',
          progress: 0,
          totalPages: pageRefs.length
        }
      })
  ...
  async retryFailed(
    ...
    if (!styleId) throw new Error('重試來源缺少目標風格')
  ...
  private async run(job: ActiveStyleSwitchJob): Promise<void> {
    try {
      ...
      await this.ctx.db.updateGenerationRunStatus(
        job.runId,
        runStatus,
        failedCount > 0 ? `${failedCount} 個頁面切換失敗` : null
      )
  ...
  private async runPage(job: ActiveStyleSwitchJob, page: StyleSwitchPageRef): Promise<void> {
    ...
    try {
      ...
      if (!pageSnapshot.exists || pageSnapshot.content === html) {
        throw new Error('當前頁面編輯沒有檢測到落盤變化。')
      }
      ...
      if (
        ...
        (historyAlreadyCommitted ||
          historyCommitFailed ||
          /index\.html|歷史記錄|history/i.test(message))
      ) {
    ...
    try {
      ...
      try {
        operation = await history.recordOperation({
          ...
          type: 'edit',
          scope: 'page',
          prompt: `切換風格 · 第 ${page.pageNumber} 頁`,
...
export function registerStyleSwitchJobHandlers(
  ...
  const interruptedReady = service
    .abortInterruptedJobs('應用退出導致風格切換中斷，可重試')
    .catch((error) => {
