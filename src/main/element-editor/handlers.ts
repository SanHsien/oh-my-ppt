/** IPC handlers for structured element editing, distinct from the HTML workspace editor. */
...
export function registerEditorHandlers(ctx: IpcContext): void {
  ...
  ipcMain.handle('element-anchor:ensure', async (_event, payload: unknown) => {
    ...
    if (!htmlPath) throw new Error('頁面路徑不能爲空')
    if (!pageId) throw new Error('pageId 不能爲空')
    if (!selector) throw new Error('元素 selector 不能爲空')
  ...
  ipcMain.handle('element-animation:set', async (_event, payload: unknown) => {
    ...
    if (!pageId) throw new Error('pageId 不能爲空')
    if (!selector) throw new Error('元素 selector 不能爲空')
    if (!patch) throw new Error('元素動畫 patch 不能爲空')
    ...
    const result = await withHtmlFileLock(safeHtmlPath, async () => {
      ...
      if (newErrors.length > 0) {
        throw new Error(`元素動畫驗證失敗：${newErrors.join('; ')}`)
      }
      if (next.changed) {
    ...
    if (result.changed) {
      await history.recordOperation({
        ...
        prompt: result.config
          ? `爲元素設置動畫：${result.config.type} ${result.config.durationMs}ms`
          : '關閉元素動畫',
        metadata: {
  ...
  ipcMain.handle('element-editor:delete-element', async (_event, payload: unknown) => {
    ...
    if (!htmlPath) throw new Error('頁面路徑不能爲空')
    if (!pageId) throw new Error('pageId 不能爲空')
    if (!selector) throw new Error('刪除元素 selector 不能爲空')
    ...
    await withHtmlFileLock(safeHtmlPath, async () => {
      ...
      if (!target || target.length === 0) {
        throw new Error('無法定位刪除元素：頁面內容可能已經變化')
      }
      target.remove()
  ...
  ipcMain.handle('edit:save-batch', async (_event, payload: unknown) => {
    ...
    const prompt = typeof record.prompt === 'string' ? record.prompt : '手動調整'
    await new GitHistoryService(db).recordOperation({
      sessionId,
      projectDir,
      type: 'edit',
  ...
  ipcMain.handle('element-editor:apply-sync-to-all-pages', async (_event, payload: unknown) => {
    ...
    if (pages.length === 0) throw new Error('沒有可同步的頁面')
  ...
  ipcMain.handle('drag-editor:update-element-layout', async (_event, payload: unknown) => {
    ...
    if (!htmlPath) throw new Error('頁面路徑不能爲空')
    if (!pageId) throw new Error('pageId 不能爲空')
    if (!selector) throw new Error('拖拽元素 selector 不能爲空')
  ...
  ipcMain.handle('text-editor:update-element-text', async (_event, payload: unknown) => {
    ...
    if (!htmlPath) throw new Error('頁面路徑不能爲空')
    if (!pageId) throw new Error('pageId 不能爲空')
    if (!selector) throw new Error('文字元素 selector 不能爲空')
    if (!text) throw new Error('文字不能爲空')
    if (text.length > 500) throw new Error('文字不能超過 500 個字符')
