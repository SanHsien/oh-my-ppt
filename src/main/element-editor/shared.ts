...
export function assertAnchorableElement(target: cheerio.Cheerio<AnyNode>): void {
  ...
  if (!tagName || BLOCKED_TAGS.has(tagName)) {
    throw new Error(`當前元素不能錨定：<${tagName || 'unknown'}>`)
  }
  ...
  if (
    ...
    throw new Error('頁面骨架元素不能錨定，請選擇頁面內容裏的具體元素')
  }
}
...
export function patchElementProperties(
  ...
  if (!EDITABLE_TEXT_TAGS.has(tagName) && !hasRole && !hasBlockId) {
    throw new Error(`當前元素暫不支持直接編輯文字：<${tagName || 'unknown'}>`)
  }
  if (!hasOnlyEditableTextChildren($, target)) {
    throw new Error('當前元素包含非文本子元素，暫不支持直接編輯；可以選擇更內層的文字。')
  }
  ...
  if (typeof patch.html === 'string') {
    ...
    if (!normalizedText) throw new Error('文字不能爲空')
    if (normalizedText.length > 500) throw new Error('文字不能超過 500 個字符')
...
function stripUnsafeFormulaHtml(
  ...
  if (!normalizedLatex) throw new Error('公式不能爲空')
  if (normalizedLatex.length > 2000) throw new Error('公式不能超過 2000 個字符')
  if (html.length > 100000) throw new Error('公式內容過長')
  const $ = cheerio.load(`<root>${html}</root>`, { scriptingEnabled: false }, false)
  const root = $('root').first()
  ...
  if (rendered.length === 0) throw new Error('公式渲染結果無效')
...
export function patchGenericElementProperties(
  ...
  if (patch.formula && typeof patch.formula.html === 'string') {
    ...
    if (sourceReplaced !== null) {
      ...
      else throw new Error('公式定位失敗，未改動原文；請重新選擇公式後再編輯')
    }
...
export function ensureElementAnchorInHtml(
  ...
  try {
    ...
    throw new Error('無法錨定元素：selector 無效')
  }
  if (!target || target.length === 0) {
    ...
    throw new Error('無法錨定元素：頁面內容可能已經變化')
  }
