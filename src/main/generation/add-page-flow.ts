...
export async function executeAddPageGeneration(
  ...
  if (!designContract) {
    throw new Error('當前會話缺少設計契約，無法新增頁面。請先完成首次生成。')
  }
  ...
  if (existingPages.length === 0) {
    throw new Error('當前會話沒有已完成的頁面，無法新增。請先完成首次生成。')
  }
  ...
  emitChunk({
    ...
    payload: {
      runId: context.runId,
      stage: 'planning',
      label: uiText(context.appLocale, '正在規劃新增頁面', 'Planning the new page'),
      progress: 2,
      totalPages: 1
    }
  })
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `規劃新頁面失敗：${planError instanceof Error ? planError.message : String(planError)}`
      )
    }
  }
  ...
  emitChunk({
    ...
    payload: {
      runId: context.runId,
      stage: 'rendering',
      label: uiText(context.appLocale, '正在生成新增頁面', 'Generating the new page'),
      progress: 10,
      totalPages: 1
    }
  })
  ...
  try {
    const generationResult = await generatePagesWithRetry({
      runArgs: {
        ...
        renderingLabel: uiText(context.appLocale, '正在生成新增頁面', 'Generating the new page'),
        pageTasks: [
          {
            pageNumber: newPageNumber,
            pageId: newPageId,
    ...
    if (!newPageValidation.valid) {
      throw new Error(`新頁面 HTML 驗證失敗: ${newPageValidation.errors.join('; ')}`)
    }
  ...
  const assistantContent =
    ...
    uiText(
      context.appLocale,
      `已新增頁面「${planResult.title}」並插入到第 ${insertAfterPageNumber} 頁之後。`,
