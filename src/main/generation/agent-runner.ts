...
const parseModelJson = (responseText: string, appLocale?: AppLocale): unknown => {
  ...
  throw new Error(
    uiText(
      appLocale,
      `LLM 返回的 JSON 解析失敗: ${lastError instanceof Error ? lastError.message : String(lastError)}. 原始文本預覽: ${preview}`,
  ...
  const parsePlanningItems = (responseText: string): OutlineItem[] => {
    ...
    if (!Array.isArray(parsed)) {
      throw new Error(
        uiText(
          args.appLocale,
          'LLM plan_deck 返回格式不正確，期望 [{title, keyPoints[], layoutIntent}] 數組。',
    ...
    if (parsed.length === 0 || typeof parsed[0] !== 'object' || parsed[0] === null) {
      throw new Error(
        uiText(
          args.appLocale,
          'LLM plan_deck pages 返回格式不正確，期望 [{title, keyPoints[], layoutIntent}] 數組。',
    ...
    const items: OutlineItem[] = (parsed as Array<Record<string, unknown>>).map((item, index) => {
      ...
      if (!title) {
        throw new Error(
          uiText(
            args.appLocale,
            `LLM plan_deck 第 ${index + 1} 項缺少 title，期望格式: { title, keyPoints[], layoutIntent }`,
      ...
      if (keyPoints.length < 1) {
        throw new Error(
          uiText(
            args.appLocale,
            `LLM plan_deck 第 ${index + 1} 項 keyPoints 爲空，至少需要 1 條。`,
    ...
    while (items.length < args.totalPages) {
      items.push({
        title: uiText(args.appLocale, `第 ${items.length + 1} 頁`, `Page ${items.length + 1}`),
        contentOutline: '',
...
export const buildDesignContractWithLLM = async (args: {
  ...
  const parseDesignContract = async (responseText: string): Promise<DesignContract> => {
    ...
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
      throw new Error(
        uiText(
          args.appLocale,
          'LLM design_contract 返回格式不正確，期望 JSON object。',
    ...
    if (!Array.isArray(record.palette) || record.palette.length < 3) {
      throw new Error(
        uiText(
          args.appLocale,
          'LLM design_contract palette 至少需要 3 個顏色。',
    ...
    if (requestedFontPair) {
      if (
        ...
        throw new Error(
          uiText(
            args.appLocale,
            `LLM design_contract 字體與用戶選擇不一致：titleFont=${contract.titleFont}, bodyFont=${contract.bodyFont}`,
  ...
  args.emit?.({
    ...
    payload: {
      ...
      provider: args.provider,
      model: args.model,
      detail: uiText(args.appLocale, '正在生成獨立設計契約', 'Generating design contract')
    }
  })
  const maxAttempts = 2
  let lastError: unknown = null
  ...
  throw new Error(
    uiText(
      args.appLocale,
      `設計契約生成失敗：${lastError instanceof Error ? lastError.message : String(lastError)}`,
...
export const runDeepAgentDeckGeneration = async (args: {
  ...
  const pageRefs: PageRef[] =
    args.pageTasks && args.pageTasks.length > 0
      ...
      : (() => {
          ...
          if (pageIds.length === 0) {
            throw new Error('pageFileMap 爲空，無法建立頁面任務。')
          }
  ...
  const resolvePageProgressFromCustomStatus = (custom: DeckToolStatusChunk): number => {
    ...
    if (/讀取會話上下文|Reading session context/i.test(label)) return 25
    if (/更新\s*page-\S+|更新單頁\s+\S+|Updating\s+\S+/i.test(label)) return 60
    if (/驗證完成狀態|Verifying completion/i.test(label)) return 85
  ...
  emitRenderingStatus({
    ...
    progress: 12,
    detail: uiText(args.appLocale, `共 ${totalPages} 頁`, `${totalPages} pages`)
  })
  ...
  const generateSinglePage = async (
    ...
    if (!currentPagePath) {
      throw new Error(`pageFileMap 缺少 ${page.pageId} 對應文件路徑`)
    }
    ...
    try {
      ...
      await processAgentStreamCore(stream, {
        ...
        onCustom: (custom) => {
          ...
          const normalizedDetail =
            /所有頁面已填充|當前頁面已填充|All pages filled|Current page filled/i.test(
              custom.label || ''
            )
              ? uiText(
                  args.appLocale,
      ...
      if (
        ...
        throw new Error(
          [
            `頁面未寫入 (${page.pageId})：模型沒有成功調用 ${writeToolName} 寫入目標 page 文件。`,
            `必須調用 ${writeToolName}(pageId="${page.pageId}", content=完整創意頁面片段)，不要只在最終回覆裏描述 HTML。`
          ].join(' ')
        )
      }
      ...
      emitPageStatus({
        pageId: page.pageId,
        label: progressLabel(args.appLocale, '頁面內容已寫入'),
        detail: `${page.pageId} · ${page.title}`,
        pageProgress: 95
      })
      ...
      emitRenderingStatus({
        ...
        detail: uiText(
          args.appLocale,
          `${page.title} · 已完成 ${completedCount}/${totalPages} 頁`,
  ...
  // 僅重試失敗頁面，避免影響已成功頁面。
  // MAX_PAGE_RETRIES=3 表示首輪失敗後最多再重試 3 次。
  ...
  const generateSinglePageWithRetry = async (
    ...
    for (let attempt = 0; attempt <= MAX_PAGE_RETRIES; attempt++) {
      try {
        ...
        const isWriteError = /落盤校驗|禁止的 CDN|遠程資源|未知頁面|不允許寫入/i.test(reason)
    ...
    throw lastError instanceof Error
      ? lastError
      : new Error(
          String(lastError ?? uiText(args.appLocale, '頁面生成失敗', 'Page generation failed'))
        )
  }
  ...
  if (useDualWorkerQueue) {
    emitRenderingStatus({
      label: renderingLabel,
      progress: 14,
      detail: uiText(args.appLocale, '創意即將正式生成..', 'Generation is about to begin.')
    })
  }
  ...
  args.emit?.({
    ...
    payload: {
      ...
      label: concurrentDeckPageId
        ? uiText(
            args.appLocale,
            `正在啓動頁面 ${concurrentDeckPageId} 的編輯`,
