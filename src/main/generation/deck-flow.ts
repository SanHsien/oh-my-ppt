...
export async function executeDeckGeneration(
  ...
  const scaffoldPromise = scaffoldProjectFiles({
    ...
    emitDeckChunk({
      ...
      payload: {
        ...
        detail: uiText(
          context.appLocale,
          `已創建 index.html 與 ${pageRefs.length} 個頁面骨架`,
  ...
  if (shouldUseSourcePlan) {
    ...
    emitDeckChunk({
      ...
      payload: {
        ...
        detail: uiText(
          context.appLocale,
          `已使用源文檔結構生成 ${pageRefs.length} 頁計劃`,
  ...
  emitDeckChunk({
    ...
    payload: {
      ...
      detail: uiText(
        context.appLocale,
        `已完成規劃並更新目錄標題，設計契約：${designContract.theme}`,
    ...
    payload: {
      ...
      detail:
        postValidationErrors.length > 0
          ...
          : uiText(
              context.appLocale,
              `全部 ${pageRefs.length} 個頁面文件都已準備完成`,
  ...
  for (const item of generatedPageReads) {
    ...
    if (isPlaceholderPageHtml(html)) {
      const reason = '頁面仍爲佔位內容，模型沒有成功寫入真實頁面'
    ...
    await db.addMessage(context.sessionId, {
      role: 'tool',
      content: `${changed ? '已更新' : '已確認'} ${page.pageId}: ${page.title}`,
      type: 'tool_result',
  ...
  if (failedPages.length > 0) {
    ...
    emitDeckChunk({
      ...
      payload: {
        ...
        detail: uiText(
          context.appLocale,
          `本次已完成 ${pageDescriptors.length}/${pageRefs.length} 頁，失敗頁面：${failedDetails}`,
          `${pageDescriptors.length}/${pageRefs.length} pages completed. Failed pages: ${failedDetails}`
        )
      }
    })
    throw new Error(
      `部分頁面生成失敗（${failedPages.length}/${pageRefs.length}）：${failedPages
  ...
  const fallbackCompletionSummary =
    placeholderPages.length > 0
      ? uiText(
          context.appLocale,
          `演示已生成完成。當前共 ${pageDescriptors.length} 頁，主題「${context.topic}」。其中 ${placeholderPages.length} 頁可以繼續優化。`,
      ...
      : uiText(
          context.appLocale,
          `演示已生成完成。共 ${pageDescriptors.length} 頁，主題「${context.topic}」。`,
