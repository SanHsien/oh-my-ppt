...
export async function executeDeckAllPageEditGeneration(
  ...
  if (context.messageScope !== 'main') {
    throw new Error('deck 全頁編輯只接受主會話消息。')
  }
  ...
  if (selectedPageRefs.length > pageLimit) {
    throw new Error(
      uiText(
        context.appLocale,
        `一次最多編輯 ${pageLimit} 頁，請先選擇更小的頁面範圍。`,
  ...
  emitEditChunk({
    ...
    payload: {
      runId: context.runId,
      stage: 'editing',
      label: uiText(context.appLocale, '正在準備批量編輯', 'Preparing batch edit'),
      progress: 10,
      totalPages: selectedPageRefs.length
    }
  })
        ...
        if (isRetry) {
          ...
          emit({
            ...
            payload: {
              ...
              label: uiText(
                context.appLocale,
                `正在重試 P${retryPage?.pageNumber ?? currentPage}`,
  ...
  const failedPageLabels = failedBatchResults.map((item) => {
    ...
    return uiText(
      context.appLocale,
      `第${page?.pageNumber || item.pageId}頁`,
