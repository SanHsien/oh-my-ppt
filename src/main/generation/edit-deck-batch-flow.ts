...
export class DeckEditIndexMutationError extends Error {
  constructor() {
    super('主會話 deck 編輯不允許修改 index.html，本次檢測到殼層變更並已恢復。')
...
const errorMessage = (error: unknown): string =>
  error instanceof Error && error.message.length > 0 ? error.message : String(error || '未知錯誤')
      ...
      label:
        reportsCompletedStep
          ? args.appLocale === 'en'
            ...
            : `P${args.pageNumber} 當前步驟完成，正在校驗頁面`
          : chunk.type === 'llm_status' &&
        /理解|分析|規劃|準備|啓動|生成|編輯|完成|understand|analyz|plan|prepar|start|generat|edit|complet/i.test(
          chunk.payload.label
        )
          ? args.appLocale === 'en'
            ? `Editing P${args.pageNumber}`
            : `正在編輯 P${args.pageNumber}`
          : chunk.payload.label,
      progress,
        ...
        emitPageProgress(
          ...
          retryUsed
            ? args.appLocale === 'en'
              ...
              : `P${page.pageNumber} 重試成功`
            : args.appLocale === 'en'
              ? `P${page.pageNumber} editing completed`
              : `P${page.pageNumber} 編輯完成`
        )
        return {
          status: 'completed',
          pageId: page.pageId,
