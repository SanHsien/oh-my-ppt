...
const buildPageEditAssessmentSystemPrompt = (locale: 'zh' | 'en', args: {
  ...
  const selectorContext = [
    ...
    .filter(Boolean)
    .join('\n')
  const localeRule = locale === 'en' ? 'Use English.' : '使用簡體中文。'
  return [
...
export async function assessPageEdit(
  ...
  if (input.requestedType !== 'page' || input.chatType !== 'page') {
    throw new Error('僅支持分析當前頁面的修改請求')
  }
  if (!input.rawUserMessage.trim()) throw new Error('請輸入頁面修改需求')
  const requestedPageId = input.chatPageId || input.selectedPageId
  if (!requestedPageId) throw new Error('頁面修改分析需要指定目標頁面')
  ...
  if (!activeModel.apiKey) {
    throw new Error(`當前 provider "${activeModel.provider}" 缺少 API Key，請先到設置頁配置。`)
  }
  ...
  if (!fs.existsSync(pagePath)) throw new Error(`目標頁面文件不存在: ${page.file_slug}.html`)
  ...
  if (!assessment) throw new Error('AI 未完成頁面修改意圖分析，請重試或補充需求。')
...
export async function executeEditGeneration(
  ...
  if (context.messageScope === 'main') {
    throw new Error('主會話編輯需要走 deck 全頁編輯流程，不能進入單頁編輯流程。')
  }
  ...
  emitEditChunk({
    ...
    payload: {
      ...
      label: resolvedSelectedPageNumber
        ? uiText(
            context.appLocale,
            `正在準備編輯第 ${resolvedSelectedPageNumber} 頁`,
            `Preparing to edit page ${resolvedSelectedPageNumber}`
          )
        : uiText(context.appLocale, '正在定位需要編輯的頁面', 'Locating pages to edit'),
      progress: 10,
      ...
      emitEditChunk({
        ...
        payload: {
          ...
          label: resolvedSelectedPageNumber
            ? uiText(
                context.appLocale,
                `正在重試第 ${resolvedSelectedPageNumber} 頁的編輯`,
                `Retrying the edit for page ${resolvedSelectedPageNumber}`
              )
            : uiText(context.appLocale, '正在重試頁面編輯', 'Retrying the page edit'),
          progress: 55,
          totalPages: editTotalPages,
          detail: retryDetail
        }
  ...
  if (invalidChangedPages.length > 0) {
    ...
    await runRetryAttempt(
      buildEditValidationRetryMessage(context.userMessage, `頁面編輯結果驗證失敗：${details}`),
      uiText(
        context.appLocale,
        '頁面校驗失敗，正在自動重試一次。',
        'Page validation failed; retrying once.'
