...
const extractStructuredTokens = (input: string): string[] => {
  const tokenMatches = input.match(
    /(?:[Pp][0-9]+)|(?:\d{4}[-/.年]\d{1,2}(?:[-/.月]\d{1,2}日?)?)|(?:\d+(?:\.\d+)?%)|(?:\d+(?:\.\d+)?(?:萬|億|元|萬元|億元)?)|(?:[A-Za-z][A-Za-z0-9_-]{1,})/g
  )
