...
export async function executeRetryFailedPages(
  ...
  if (retryRecords.length === 0) {
    throw new Error('當前會話沒有可繼續生成的頁面。')
  }
  if (completedSessionPageCount === 0) {
    throw new Error('當前沒有成功頁面可保留，請使用完整重新生成。')
  }
  if (
  ...
  emitRetryChunk({
    ...
    payload: {
      ...
      label: uiText(
        context.appLocale,
        `正在重新生成 ${retryPages.length} 個失敗頁面`,
  ...
  const { summary: agentSummary, failedPages } = await runDeepAgentDeckGeneration({
    ...
    userMessage:
      context.userMessage ||
      [
        '繼續生成本會話中未完成的頁面。頁面正文、標題、圖表標籤必須保持與現有頁面相同語言。',
    ...
    if (failedPageIdSet.has(page.pageId)) {
      ...
      if (!persistedRetryFailedPageIds.has(page.pageId)) {
        await db.upsertGenerationPage({
          ...
          htmlPath: page.htmlPath,
          status: 'failed',
          error: failure?.reason || '頁面重試失敗',
          retryCount: page.retryCount
        })
        await upsertRetrySessionPage(page, 'failed', failure?.reason || '頁面重試失敗')
        persistedRetryFailedPageIds.add(page.pageId)
      }
      continue
    }
  ...
  if (retryFailures.length > 0) {
    ...
    throw new Error(
      `重試後仍有頁面失敗（${retryFailures.length}/${retryPages.length}）：${retryFailures
  ...
  if (mergedGeneratedPages.length < sessionPages.length) {
    const message = uiText(
      context.appLocale,
      `重試頁面已完成，但當前只恢復 ${mergedGeneratedPages.length}/${sessionPages.length} 頁，請繼續重試或重新生成。`,
  ...
  const fallbackCompletionSummary = uiText(
    context.appLocale,
    `失敗頁面已經重試完成，本次修復 ${retrySuccessPages.length} 頁。`,
