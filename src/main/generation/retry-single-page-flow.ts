...
export async function executeRetrySinglePageGeneration(
  ...
  if (!designContract) {
    throw new Error('當前會話缺少設計契約，無法重試。')
  }
...
<main data-role="content"><p>Regenerating...</p></main>
    ...
    generationResult = await generatePagesWithRetry({
      runArgs: {
        ...
        userMessage: `重新生成第 ${context.pageNumber} 頁「${context.title}」`,
...
<main data-role="content"><p>Retrying...</p></main>
      ...
      buildRetryRunArgs: (runArgs) => ({
        ...runArgs,
        userMessage: `重新生成第 ${context.pageNumber} 頁「${context.title}」。如果需要圖表，先 ${formatSkillUsageRequirement(CHART_SKILL_NAME)}`
      })
    })
    ...
    if (!validation.valid) {
      throw new Error(`重試頁面 HTML 驗證失敗: ${validation.errors.join('; ')}`)
    }
  } catch (error) {
    try {
  ...
  await ctx.history.recordOperation({
    ...
    type: 'retry',
    scope: 'page',
    prompt: `重新生成第 ${context.pageNumber} 頁「${context.title}」`,
    metadata: {
      runId: context.runId,
      pageId: context.pageId
    }
