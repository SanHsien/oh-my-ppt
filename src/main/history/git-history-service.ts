...
export class GitHistoryService {
  ...
  async recordOperation(args: RecordOperationArgs): Promise<SessionOperationRecord | null> {
    ...
    if (changedFiles.length === 0 && args.allowEmptySnapshot && beforeCommit) {
      ...
      if (!hasRestorableDeckFiles(trackedFiles)) {
        throw new Error('歷史記錄寫入失敗：未記錄到可恢復的頁面文件。')
      }
    ...
    try {
      ...
      if (!hasRestorableDeckFiles(trackedAfterCommit)) {
        throw new Error('歷史記錄寫入失敗：提交後未記錄到可恢復的頁面文件。')
      }
      await this.db.completeSessionOperation({
        id: operationId,
        status: 'completed',
  ...
  async rollbackCommittedOperation(args: {
    ...
    if (!beforeCommit) throw new Error('歷史補償失敗：缺少提交前版本。')
    const projectDir = path.resolve(args.projectDir)
  ...
  async rollbackToVersion(args: {
    ...
    if (session?.status === 'active') {
      throw new Error('當前會話正在生成或編輯，暫時不能回退。')
    }
    ...
    if (!beforeCommit) {
      throw new Error('當前會話尚未建立歷史記錄，不能回退。')
    }
    ...
    if (!hasRestorableDeckFiles(operationTrackedFiles)) {
      throw new Error('目標歷史版本記錄不完整（tracked_files_json 缺少頁面文件），無法回退。')
    }
  ...
  private async syncSessionPagesForRestoredVersion(
    ...
    if (order.length === 0) {
      throw new Error('目標歷史版本缺少頁面快照，無法恢復頁面順序。')
    }
    ...
    for (let index = 0; index < order.length; index += 1) {
      ...
      if (!(typeof item.pageId === 'string' && item.pageId.trim().length > 0)) {
        throw new Error('目標歷史版本頁面快照缺少 pageId，無法恢復頁面順序。')
      }
      ...
      await this.db.upsertSessionPage({
        ...
        error: restoredStatus === 'failed' ? existing?.error || '頁面文件不存在' : null
      })
    }
  ...
  private async createLegacyImport(sessionId: string, projectDir: string): Promise<void> {
    ...
    if (
      ...
      throw new Error('舊會話文件不完整，無法建立歷史起點。')
    }
  ...
  private async assertCommitExists(projectDir: string, commit: string): Promise<void> {
    try {
      ...
      throw new Error('目標歷史版本對應的提交對象不存在，無法回退。')
    }
  }
  ...
  private async restoreCommitFiles(
    ...
    if (!hasRestorableDeckFiles(normalizedTargetFiles)) {
      throw new Error('目標歷史版本缺少可恢復的頁面文件，無法回退。')
    }
  ...
  private titleForOperation(
    ...
    if (metadata.jobType === 'style-switch') {
      return Number.isInteger(styleSwitchPageNumber) && styleSwitchPageNumber > 0
        ? `切換風格 · 第 ${styleSwitchPageNumber} 頁`
        : '切換風格'
    }

    if (effectiveMode === 'addPage' || metadata.addPage === true) return '新增頁面'
    if (effectiveMode === 'retrySinglePage') return '重試頁面'
    if (effectiveMode === 'retry') return '重試失敗頁面'

    if (operation.type === 'import' && metadata.legacy) return '歷史起點'
    if (type === 'import') return '導入 PPTX'
    if (type === 'generate') return '首次生成'
    if (type === 'addPage' || type === 'add_page') return '新增頁面'
    if (type === 'reorder') return '調整頁面順序'
    if (type === 'delete') return '刪除頁面'
    if (type === 'retry') return scope === 'page' ? '重試頁面' : '重試失敗頁面'
    if (type === 'rollback') return '回退到歷史版本'
    if (type === 'edit') {
      if (scope === 'deck') return '全局修改頁面'
      if (scope === 'selector') return '局部修改頁面元素'
      if (scope === 'page') return '編輯頁面'
      if (scope === 'session') return '調整頁面'
      if (scope === 'shell') return '調整頁面容器'
    }
    return '歷史版本'
  }
  ...
  private descriptionForOperation(
    ...
    if (operation.type === 'rollback') return '已恢復到選定版本'
    return '已記錄此時間點'
  }
}
