...
const resolvePageContext = async (
  ...
  if (!page) throw new Error('請先選擇一個可用頁面。')
...
export function registerImageGenerationHandlers(
  ...
  const throwIfCancelled = (signal: AbortSignal, locale: AppLocale): void => {
    if (signal.aborted) {
      throw new Error(uiText(locale, '已取消生圖', 'Image generation cancelled'))
    }
  }
  ...
  ipcMain.handle('images:generatePrompt', async (_event, payload) => {
    ...
    if (!sessionId) throw new Error(uiText(locale, '會話 ID 不能爲空。', 'Session ID is required.'))
    if (!htmlPath) {
      throw new Error(
        uiText(locale, '當前頁文件地址不能爲空。', 'Current page file path is required.')
      )
    }
    ...
    if (!pageHtml) {
      throw new Error(uiText(locale, '當前頁內容爲空。', 'Current page content is empty.'))
    }
    ...
    if (!prompt) {
      throw new Error(uiText(locale, '模型未返回提示詞。', 'The model returned an empty prompt.'))
    }
  ...
  ipcMain.handle('images:generate', async (_event, payload) => {
    ...
    if (!sessionId) throw new Error(uiText(locale, '會話 ID 不能爲空。', 'Session ID is required.'))
    if (!pageId) throw new Error(uiText(locale, '請先選擇頁面。', 'Select a page first.'))
    if (!prompt) throw new Error(uiText(locale, '請先填寫圖片描述。', 'Enter an image prompt first.'))
    ...
    const initialState: ImageRunState = {
      ...
      pageId,
      progress: 5,
      label: uiText(locale, '準備生圖', 'Preparing image generation'),
      status: 'running',
      updatedAt: Date.now()
    }
    ...
    try {
      ...
      updateImageRunState({
        ...
        pageId: pageContext.pageId,
        progress: 80,
        label: uiText(locale, '正在保存圖片', 'Saving images'),
        status: 'running',
        updatedAt: Date.now()
      })
      ...
      updateImageRunState({
        ...
        pageId: pageContext.pageId,
        progress: 100,
        label: uiText(locale, '生圖完成', 'Image generation completed'),
        status: 'completed',
        updatedAt: Date.now()
      })
      ...
      updateImageRunState({
        ...
        pageId: resolvedPageId,
        progress: 100,
        label: wasCancelled ? uiText(locale, '已取消生圖', 'Image generation cancelled') : message,
