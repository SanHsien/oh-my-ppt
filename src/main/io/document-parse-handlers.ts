...
const assertPlanLanguageMatchesSource = async (args: {
  ...
  if (isMostlyChineseText(sourceText) && ENGLISH_BRIEF_LABEL_PATTERN.test(args.plan.briefText)) {
    throw new RetryableDocumentPlanQualityError(
      '源文檔主要是中文，但 briefText 使用了英文結構標籤。請用中文結構標籤返回，例如：演示目標、受衆/場景、核心觀點、建議大綱、每頁要點、必須保留的事實/指標/術語、風格/表達要求。不要使用 Presentation goal、Audience/context、Core argument、Recommended outline、Per-page points、Page 1 等英文模板標籤。'
    )
  }
}
...
const prepareSourceFile = async (
  ...
  if (!SUPPORTED_EXTENSIONS.has(ext) && !isImage) {
    throw new Error('暫只支持 md、txt、csv、docx 文檔，以及 png、jpg、jpeg、webp 圖片')
  }
...
const assertPlanMatchesDocumentOutline = (args: {
  ...
  const hasSourceHeadingLabel =
    /源文檔結構|來源標題|Source document structure|Source heading/i.test(briefText)
...
const normalizeLightweightGeneratedPlan = (
  ...
  const pageCount = Number.isFinite(rawPageCount)
    ...
    : fallback.pageCount
  if (!topic) throw new Error('文檔解析完成，但模型未返回 topic')
  return {
    topic,
    pageCount,
    briefText: ''
...
const formatPageSkeletonBriefText = (args: {
  ...
  const outlineLabel = useChineseLabels ? '建議大綱' : 'Recommended outline'
  const summaryLabel = useChineseLabels ? '簡要總結' : 'Brief summary'
  const pageLabel = useChineseLabels ? '第' : 'Page'
  const pageSuffix = useChineseLabels ? ' 頁' : ''
...
const buildSingleShotDocumentPlanPrompt = (args: {
  ...
  return [
    ...
    '- Keep chapter divider slides as standalone section-divider pages and include a role marker such as 頁面角色：章節頁 or Page role: chapter divider.',
    ...
    useLightweightSourcePlan
      ? '{"topic":"直播與短視頻自然流增長：汽車經銷商新媒體實戰指南","pageCount":65}'
      : '',
    useLightweightSourcePlan
      ? ''
      : 'For Chinese source: {"topic":"直播與短視頻自然流增長——汽車經銷商新媒體實戰指南","pageCount":65,"briefText":"演示目標：...\\n源文檔結構：...\\n建議大綱：\\n1. 手冊結構導航\\n2. 閱讀角色指引\\n每頁要點：\\n第 1 頁：手冊結構導航\\n頁面角色：內容頁\\n來源標題：### 手冊結構導航\\n來源範圍：lines 17-32\\n頁面目的：說明本手冊的結構導航。"}',
    useLightweightSourcePlan
      ? ''
...
const summarizePageSummaryBatch = async (args: {
  ...
  for (let attempt = 1; attempt <= PAGE_SUMMARY_BATCH_MAX_ATTEMPTS; attempt += 1) {
    ...
    try {
      ...
      throw new Error(`模型摘要返回缺少頁面 ID: ${missingIds.join(', ')}`)
    } catch (error) {
      lastError = error
  ...
  throw lastError || new Error('模型摘要批次失敗')
}
...
const buildImageDocumentPlanPrompt = (args: {
  ...
  [
    ...
    '- If the image is primarily Chinese, use Chinese section labels such as 演示目標、受衆/場景、核心觀點、建議大綱、每頁要點、必須保留的事實/指標/術語、風格/表達要求.',
    ...
    '',
    'Return format examples:',
    '{"topic":"AI動漫產業發展分析","pageCount":7,"briefText":"演示目標：...\\n受衆/場景：...\\n核心觀點：...\\n建議大綱：\\n1. ...\\n每頁要點：\\n第 1 頁：...\\n必須保留的事實/指標/術語：...\\n風格/表達要求：..."}',
...
const writeImagePlanReferenceFile = async (args: {
  ...
  if (!briefText) throw new Error('圖片解析完成，但模型未返回可用參考內容')
  const useChineseLabels = isMostlyChineseText(`${args.plan.topic}\n${briefText}`)
  const markdown = [
    `# ${path.basename(args.file.name, ext) || (useChineseLabels ? '圖片參考' : 'Image reference')}`,
    '',
    ...
    '',
    `## ${useChineseLabels ? '主題' : 'Topic'}`,
    '',
    args.plan.topic,
    '',
    `## ${useChineseLabels ? '建議頁數' : 'Suggested page count'}`,
    '',
    String(args.plan.pageCount),
    '',
    `## ${useChineseLabels ? '圖片解析參考' : 'Image analysis reference'}`,
    '',
    briefText
  ].join('\n')
...
const convertImageReferenceToMarkdown = async (args: {
  ...
  if (!mimeType) throw new Error('暫只支持 png、jpg、jpeg、webp 圖片')
  ...
  if (!content) throw new Error('圖片解析完成，但模型未返回可用內容')

  const mdPath = args.file.workspacePath.replace(/\.[^.]+$/, '.image.md')
  await fs.promises.writeFile(
    mdPath,
    [
      `# ${path.basename(args.file.name, ext) || '圖片參考'}`,
      '',
      `> Source image: ${args.file.name}`,
...
export function registerDocumentParseHandlers(ctx: IpcContext): void {
  ...
  ipcMain.handle(
    ...
    async (_event, payload: PrepareReferenceDocumentPayload) => {
      ...
      if (files.length === 0) throw new Error('請先選擇要附加的參考文件')
  ...
  ipcMain.handle(
    ...
    async (_event, payload: ParseImageReferencePayload) => {
      ...
      if (!rawFile) throw new Error('請先選擇要解析的圖片')

      const docsDir = path.join(await resolveStoragePath(), 'docs')
      await fs.promises.mkdir(docsDir, { recursive: true })
      const sourceFile = await prepareSourceFile(rawFile, docsDir)
      if (sourceFile.type !== 'image') throw new Error('請選擇 png、jpg、jpeg、webp 圖片')
  ...
  ipcMain.handle('documents:parsePlan', async (_event, payload: ParseDocumentPlanPayload) => {
    ...
    try {
      const input = payload && typeof payload === 'object' ? payload : { files: [] }
      const files = Array.isArray(input.files) ? input.files.slice(0, MAX_DOCUMENT_FILES) : []
      if (files.length === 0) throw new Error('請先選擇要解析的文檔')
      ...
      if (!sourceFile) throw new Error('請先選擇要解析的文檔')
      ...
      for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
        ...
        if (!responseText) {
          lastError = new Error('文檔解析完成，但模型未返回可用內容')
      ...
      if (!plan) throw lastError || new Error('文檔解析完成，但模型未返回可用解析結果')
      const resultFiles =
        sourceFile.type === 'image'
