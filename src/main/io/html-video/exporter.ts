...
const bundledFfmpegFileName = (): string => 'ffmpeg.exe'
...
const runFfmpeg = async (args: {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    child.on('close', (code, signal) => {
      ...
      reject(
        new Error(
          `ffmpeg 編碼失敗（退出碼 ${code ?? 'unknown'}${signal ? `，信號 ${signal}` : ''}）${
            hint ? `：${hint}` : ''
          }`
        )
      )
...
export const exportHtmlPagesToVideo = async (
  ...
  if (options.pages.length === 0) {
    throw new Error('沒有可導出的視頻頁面')
  }

  const ffmpegPath = await resolveBundledFfmpegPath()
  if (!ffmpegPath) {
    throw new Error('視頻編碼器缺失，無法導出視頻。請確認 resources/ffmpeg 中包含當前平臺的 ffmpeg。')
  }
  ...
  try {
    for (const [pageIndex, page] of options.pages.entries()) {
      ...
      if (readyResult.timedOut) {
        warnings.push(`頁面 ${page.pageId} 未收到打印就緒信號，已按當前狀態導出`)
      }
