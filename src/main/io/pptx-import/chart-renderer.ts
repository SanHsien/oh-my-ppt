...
export const unsupportedChartWarning = (blockId: string, chartType: string): string =>
  `圖表 ${blockId}（${chartType || 'unknown'}）暫不支持結構化導入，已作爲佔位導入`
...
const buildChartPlaceholderHtml = (args: {
  ...
  `<section data-block-id="${escapeHtml(args.blockId)}" data-pptx-kind="chart" data-pptx-import-mode="placeholder" data-pptx-chart-type="${escapeHtml(args.element.chartType || 'unknown')}"${args.animationAttrText} style="${args.frameStyle};display:flex;align-items:center;justify-content:center;color:#6b7280;">圖表已作爲佔位導入</section>`
...
export const buildChartBlock = (args: {
  ...
  if (/3DChart/i.test(args.element.chartType)) {
    args.warnings?.push({
      pageNumber: args.pageNumber,
      message: `圖表 ${args.blockId} 的 3D 效果已簡化爲二維圖表`
    })
  }
  let labels: string[] = []
