...
const parsePayload = (
  ...
  if (!filePath) throw new Error('PPTX 文件路徑不能爲空')
...
export function registerPptxImportHandlers(ctx: IpcContext): void {
  ...
  ipcMain.handle('pptx:import', async (event, payload: unknown) => {
    ...
    if (stat.size > MAX_PPTX_SIZE) {
      throw new Error(`PPTX 文件不能超過 ${MAX_PPTX_SIZE_MB}MB`)
    }
    ...
    const title =
      parsedPayload.title || path.basename(originalFileName, path.extname(originalFileName)) || '導入的 PPTX'
    ...
    try {
      ...
      await recordHistoryOperationStrict(db, {
        ...
        type: 'import',
        scope: 'session',
        prompt: `導入 PPTX：${originalFileName}`,
        metadata: {
          runId,
          source: 'pptx-import',
          originalFileName,
