...
const hasDeckTitleKeyword = (text: string): boolean =>
  /(總結|彙報|報告|計劃|規劃|方案|覆盤|目錄|概述|情況|不足|introduction|overview|summary|agenda|conclusion|plan|report|review)/i.test(text)
...
const adjustTextBlockWithPretext = async (args: {
  ...
  args.warnings?.push({
    ...
    message: `文本塊 ${args.blockId} 已按 Pretext 測量調整排版`
  })
...
const titleFromSlide = (slide: Slide, pageNumber: number): string => {
  ...
  return title?.slice(0, 80) || `第 ${pageNumber} 頁`
}
...
const buildImageBlock = async (args: {
  ...
  const animationAttrText = animationAttrs ? ` ${animationAttrs}` : ''
  if (!source) {
    return `<section data-block-id="${escapeHtml(args.blockId)}"${animationAttrText} style="${css};align-items:center;justify-content:center;background:#f3f4f6;color:#6b7280;font-size:18px;">圖片未能導入</section>`
  }
...
const buildTableBlock = (args: {
  ...
  const animationAttrText = animationAttrs ? ` ${animationAttrs}` : ''
  if (!rows.length) {
    return `<section data-block-id="${escapeHtml(args.blockId)}" data-pptx-kind="table" data-pptx-import-mode="placeholder"${animationAttrText} style="${placeholderCss};display:flex;align-items:center;justify-content:center;color:#6b7280;">表格已作爲佔位導入</section>`
  }
...
const buildSlideHtml = async (args: {
  ...
  for (const [index, element] of elements.entries()) {
    try {
      ...
      warnings.push({
        pageNumber: args.pageNumber,
        message: `元素 ${index + 1} 導入失敗：${error instanceof Error ? error.message : String(error)}`
      })
    }
  }
  if (!titleAssigned) {
  ...
  const contentOutline = flattenElements(elements)
    .map(({ element, text }) => {
      ...
      if (element.type === 'chart') return '圖表'
      if (element.type === 'image') return '圖片'
      return ''
    })
    .filter(Boolean)
    .slice(0, 8)
...
/**
 * 等距抽樣：從 slides 中均勻選取 count 頁，保證首尾都包含，中間按等距取。
 */
...
export async function importPptxToEditableHtml(args: {
  ...
  const title = (args.title || path.basename(fileName, path.extname(fileName)) || '導入的 PPTX').trim()
  const indexPath = path.join(args.projectDir, 'index.html')
  const imagesDir = path.join(args.projectDir, 'images')
  await fs.promises.mkdir(imagesDir, { recursive: true })
  args.onProgress?.({ stage: 'reading', progress: 5, label: '正在讀取 PPTX 文件' })
  const buffer = await fs.promises.readFile(args.filePath)
  args.onProgress?.({ stage: 'parsing', progress: 14, label: '正在解析 PPTX 結構' })
  ...
  if (slides.length === 0) {
    throw new Error('PPTX 中沒有可導入的幻燈片')
  }
  ...
  try {
    for (let i = 0; i < effectiveSlides.length; i += 1) {
      ...
      args.onProgress?.({
        ...
        label: `正在導入並校驗第 ${pageNumber} / ${effectiveSlides.length} 頁`,
        pageNumber,
        totalPages: effectiveSlides.length
      })
  ...
  args.onProgress?.({ stage: 'index', progress: 90, label: '正在生成演示總覽' })
  ...
  return {
    title: title.slice(0, 120) || '導入的 PPTX',
    pageCount: pages.length,
    indexPath,
    pages,
    warnings: allWarnings.map((warning) =>
      warning.pageNumber ? `第 ${warning.pageNumber} 頁：${warning.message}` : warning.message
    )
  }
}
