 ...
 * Presentation-domain capability; legacy tools/font-registry remains a forwarding shim.
 */
...
async function ensureGoogleFontsForProject(
  ...
  for (const family of uniqueFamilies) {
    ...
    try {
      ...
      throw new Error(`內置字體文件缺失：${family}（期望目錄：${sourceDir}）`)
    }
    if (woff2Files.length === 0) {
      throw new Error(`內置字體目錄爲空：${family}`)
    }
    await fs.promises.mkdir(targetDir, { recursive: true })
...
export async function resolveProjectFontResources(
  ...
  for (const family of families) {
    await assertFontFamilyAvailable(family, '母版字體')
    const google = GOOGLE_FONTS[family]
    if (google) {
      ...
      if (files.length === 0) throw new Error(`內置字體目錄爲空：${family}`)
      assets.push(
    ...
    if (!uploaded) throw new Error(`字體不在可用字體列表中：${family}`)
