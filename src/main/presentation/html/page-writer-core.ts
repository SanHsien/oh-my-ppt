...
const STRUCTURAL_FRAGMENT_ERROR_RE =
  /HTML 末尾存在未閉合標籤|開閉標籤數量不一致|閉標籤多於開標籤|缺少結尾|缺少 <\/body>/i
...
export function replacePageContentFragment(args: {
  ...
  if (remoteResources.length > 0) {
    throw new Error(
      `檢測到禁止的 CDN/遠程資源引用 (${args.pageId})，僅允許使用系統預注入的本地 ./assets/*。`
    )
  }
  ...
  if (!normalizedValidation.valid) {
    throw new Error(
      `HTML 驗證失敗 (${args.pageId}): ${normalizedValidation.errors.join('; ')}。請修正後重試。`
    )
  }
  ...
  if (!contentNode.length) {
    throw new Error(
      `無法定位頁面主體容器 (${args.pageId})：頁面骨架已被破壞，請先修復頁面後再編輯。`
    )
  }
  ...
  if (!persistedValidation.valid) {
    throw new Error(
      `HTML 落盤校驗失敗 (${args.pageId}): ${persistedValidation.errors.join('; ')}。請修正頁面片段後重試。`
    )
  }
...
export async function buildPersistedPageHtmlFromFragment(args: {
  ...
  if (remoteResources.length > 0) {
    throw new PageWriteValidationError(
      ...
      remoteResources,
      [
        `檢測到禁止的 CDN/遠程資源引用 (${args.pageId})，已拒絕寫入。`,
        '請移除所有 script/link 的 http(s) 或 // 外鏈，僅使用系統預注入的本地 ./assets/* 資源。',
        '示例命中：',
        ...remoteResources
      ].join('\n')
    )
  ...
  if (args.preserveTemplateSkeleton) {
    ...
    if (missingTemplateRefs.length > 0) {
      throw new PageWriteValidationError(
        ...
        missingTemplateRefs,
        [
          `模板骨架資源丟失 (${args.pageId})：${missingTemplateRefs.slice(0, 8).join(', ')}`,
          '請重新讀取目標模板頁，把背景圖、紋理、裝飾圖、mask/overlay 或 CSS url(...) 對應結構保留在 update_template_page_file 的 content 中。'
        ].join(' ')
      )
    }
  }
