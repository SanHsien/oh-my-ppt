...
const extractZipToDirectory = async (
  ...
  const files = tryReadZip(zipData)
  if (!files) throw new Error('無法讀取 ZIP 文件，請確認文件未損壞。')
  ...
  for (const [rawName, data] of Object.entries(files)) {
    ...
    if (fileCount > MAX_EXTRACTED_FILES) throw new Error('導入包文件數量過多，請精簡後重試。')
    if (totalBytes > MAX_EXTRACTED_BYTES) throw new Error('導入包解壓後體積過大，請精簡素材後重試。')
  ...
  if (entries.length === 0) throw new Error('導入包爲空或不包含可導入的會話文件。')
  if (mode === 'single-session-directory') {
    if (illegalRootFiles.length > 0 || rootNames.size !== 1) {
      ...
      throw new Error('ZIP 根目錄必須只包含一個完整會話目錄，請壓縮 session-id 文件夾後再導入。')
    }
  }
  ...
  for (const entry of entries) {
    ...
    ensureInside(targetPath, targetDir, '壓縮包包含非法路徑，已拒絕導入。')
...
const assertValidFileSlug = (value: string): string => {
  ...
  if (!/^[a-zA-Z0-9_-]+$/.test(trimmed)) {
    throw new Error(`頁面 ID 不合法：${value}`)
  }
  return trimmed
}
...
const validateSessionRoot = async (
  ...
  const pages = pagesData.map((page, index) => {
    ...
    if (path.isAbsolute(htmlFileName) || htmlFileName.includes('\\')) {
      throw new Error(`頁面路徑不合法：${htmlFileName}`)
    }
    const htmlPath = path.resolve(sessionRoot, htmlFileName)
    ensureInside(htmlPath, sessionRoot, '頁面文件路徑越界，已拒絕導入。')
    if (!htmlPath.toLowerCase().endsWith('.html')) throw new Error(`頁面文件不是 HTML：${htmlFileName}`)
    if (!fs.existsSync(htmlPath)) throw new Error(`頁面文件缺失：${htmlFileName}`)
    return {
...
const copyDirectory = async (sourceDir: string, targetDir: string): Promise<void> => {
  ...
  const copyRecursive = async (currentSource: string): Promise<void> => {
    ...
    for (const entry of entries) {
      ...
      ensureInside(targetPath, targetRoot, '複製目標路徑不合法，已拒絕導入。')
...
const assertImportedSessionReady = async (
  ...
  if (session?.status !== 'completed') throw new Error('導入校驗失敗：會話狀態未完成。')
  if (path.resolve(project?.root_path || '') !== path.resolve(args.projectDir)) {
    throw new Error('導入校驗失敗：項目目錄未正確寫入。')
  }
  if (pages.length !== args.pageCount) throw new Error('導入校驗失敗：頁面數量不一致。')
  if (!pages.every((page) => page.status === 'completed' && fs.existsSync(page.html_path))) {
    throw new Error('導入校驗失敗：頁面文件不完整。')
  }
  if (run?.mode !== 'import' || run.status !== 'completed') {
    throw new Error('導入校驗失敗：導入運行記錄不完整。')
  }
  if (history[0]?.type !== 'import' || !currentCommit) {
    throw new Error('導入校驗失敗：歷史起點未正確創建。')
  }
...
export async function importSessionFile(
  ...
  const fallbackTitle = sanitizeTitle(path.basename(originalFileName, path.extname(originalFileName)), '導入的會話')
  ...
  try {
    ...
    await recordHistoryOperationStrict(ctx.db, {
      ...
      type: 'import',
      scope: 'session',
      prompt: `導入會話文件：${originalFileName}`,
      metadata: {
        runId,
        source: 'session-file-import',
        importKind: prepared.importKind,
