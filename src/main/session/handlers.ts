...
export function registerSessionHandlers(ctx: IpcContext): void {
  ...
  ipcMain.handle('session:setIndexTransition', async (_event, payload: unknown) => {
    ...
    if (indexErrors.length > 0) {
      throw new Error(`index.html 驗證失敗: ${indexErrors.join('; ')}`)
    }
    if (next !== current) {
      ...
      await new GitHistoryService(db).recordOperation({
        ...
        prompt:
          config.type === 'none'
            ? '關閉切頁動畫'
            : `配置切頁動畫：${config.type} ${config.durationMs}ms`,
        metadata: {
          transition: config,
  ...
  ipcMain.handle('session:create', async (_event, payload) => {
    ...
    if (visualEnabled) {
      ...
      try {
        ...
        throw new Error(
          uiText(locale, `生圖模型不可用：${message}`, `The selected image model is unavailable: ${message}`)
        )
      }
    }
    ...
    if (!hasStyleSkill(normalizedStyleId)) {
      throw new Error(
        uiText(
          locale,
          `創建會話失敗：styleId 不存在 ${normalizedStyleId}`,
  ...
  ipcMain.handle('session:updateTitle', async (_event, payload: unknown) => {
    ...
    if (!title) throw new Error(uiText(locale, '會話名稱不能爲空', 'Session title is required.'))
    if (title.length > 120) {
      throw new Error(
        uiText(locale, '會話名稱不能超過 120 個字符', 'Session title cannot exceed 120 characters.')
      )
    }
    const existingSession = await db.getSession(sessionId)
    if (!existingSession) {
      throw new Error(
        uiText(locale, '會話不存在或已被刪除', 'The session does not exist or has been deleted.')
      )
    }
