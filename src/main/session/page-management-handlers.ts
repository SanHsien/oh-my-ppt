...
export function registerPageManagementHandlers(ctx: IpcContext): void {
  ...
  ipcMain.handle('session:reorderPages', async (_event, payload) => {
    ...
    const operationPrompt =
      movedPages.length > 0
        ? `調整頁面順序：${movedPreview}${movedPages.length > 2 ? `；等 ${movedPages.length} 項` : ''}`
        : '調整頁面順序（位置未變化）'
  ...
  ipcMain.handle('session:deletePages', async (_event, payload) => {
    ...
    const deletePrompt =
      deletedPages.length > 0
        ? `刪除頁面：${deletedPreview}${deletedPages.length > 3 ? `；等 ${deletedPages.length} 頁` : ''}`
        : `刪除頁面：${pageIds.length} 頁`
    await ensureHistoryBaselineSafe(ctx.db, sessionId, projectDir)
  ...
  ipcMain.handle('session:createBlankPage', async (_event, payload) => {
    ...
    if (!sessionId) throw new Error('sessionId 不能爲空')
    if (!sourcePageId) throw new Error('sourcePageId 不能爲空')
    ...
    const prompt = sourcePage
      ? `新增空白頁：複製 P${sourcePage.pageNumber}《${sourcePage.title}》`
      : '新增空白頁'
    await recordHistoryOperationStrict(ctx.db, {
      sessionId,
      type: 'addPage',
  ...
  ipcMain.handle('session:duplicatePage', async (_event, payload) => {
    ...
    const prompt = sourcePage
      ? `複製頁面：P${sourcePage.pageNumber}《${sourcePage.title}》`
      : '複製頁面'
    await recordHistoryOperationStrict(ctx.db, {
      sessionId,
      type: 'addPage',
  ...
  ipcMain.handle('session:updatePageTitle', async (_event, payload) => {
    ...
    if (!title) throw new Error('頁面標題不能爲空')

    const { projectDir, pages } = await loadEditableSessionPages(ctx, sessionId)
    const page = pages.find((item) => item.id === pageId || item.pageId === pageId)
    if (!page) throw new Error('未找到要修改標題的頁面')
    if (page.title === title) {
      return {
        ok: true,
    ...
    const prompt = `修改頁面標題：P${page.pageNumber}《${page.title}》->《${title}》`
    await recordHistoryOperationStrict(ctx.db, {
      sessionId,
      type: 'edit',
      scope: 'page',
  ...
  ipcMain.handle('session:updatePageOutline', async (_event, payload) => {
    ...
    const { projectDir, pages } = await loadEditableSessionPages(ctx, sessionId)
    const page = pages.find((item) => item.id === pageId || item.pageId === pageId)
    if (!page) throw new Error('未找到要修改大綱的頁面')
    ...
    const prompt = `修改頁面大綱：P${page.pageNumber}《${page.title}》`
    await recordHistoryOperationStrict(ctx.db, {
      sessionId,
      type: 'edit',
      scope: 'page',
