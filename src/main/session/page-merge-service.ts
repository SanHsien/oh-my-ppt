...
import { buildFontHeadTags } from '../presentation/fonts/font-registry'
import { normalizeDesignContract } from '../presentation/design-contract'
...
const copyPageResources = async (args: {
  ...
  if (unsafeReferences.length > 0) {
    throw new PageMergeError(
      ...
      `頁面包含越界資源路徑: ${unsafeReferences.join(', ')}`
    )
  }
...
const resolveTargetFontProfile = async (args: {
  ...
  if (!profile) {
    throw new PageMergeError('PAGE_MERGE_TARGET_FONT_UNAVAILABLE', '無法讀取當前模板字體配置')
  }
...
export async function listMergeSourceTemplatePages(
  ...
  const targetSession = await ctx.db.getSession(targetSessionId)
  if (!targetSession) {
    throw new PageMergeError('PAGE_MERGE_SESSION_NOT_FOUND', '當前會話不存在')
  }
  const targetSlideSize = requireSessionSlideSize(targetSession)
...
async function loadSessionMergeSource(
  ...
  if (!sourceSession) {
    throw new PageMergeError('PAGE_MERGE_SESSION_NOT_FOUND', '源會話不存在')
  }
  ...
  if (
    ...
    throw new PageMergeError('PAGE_MERGE_SESSION_BUSY', '源會話正在生成，暫時不能添加頁面')
  }
  ...
  const selectedSourcePages = sourcePageIds.map((id) => {
    ...
    if (!page) {
      throw new PageMergeError('PAGE_MERGE_SOURCE_PAGE_NOT_FOUND', `源頁面不存在: ${id}`)
    }
...
async function loadTemplateMergeSource(
  ...
  const selectedSourcePages = sourcePageIds.map((id) => {
    const page = sourcePageMap.get(id)
    if (!page) {
      throw new PageMergeError('PAGE_MERGE_SOURCE_PAGE_NOT_FOUND', `模板頁面不存在: ${id}`)
    }
    const safeHtmlPath = fs.existsSync(page.htmlPath)
      ? fs.realpathSync.native(page.htmlPath)
      : null
...
export async function mergeSessionPages(
  ...
  if (!sourceId) {
    throw new PageMergeError('PAGE_MERGE_INVALID_REQUEST', '缺少來源標識')
  }
  if (sourceType === 'session' && args.targetSessionId === sourceId) {
    throw new PageMergeError('PAGE_MERGE_SAME_SESSION', '不能從當前會話添加頁面')
  }
  const uniquePageIds = Array.from(
    new Set(args.sourcePageIds.map((item) => item.trim()).filter(Boolean))
  )
  if (uniquePageIds.length === 0) {
    throw new PageMergeError('PAGE_MERGE_NO_PAGE_SELECTED', '請選擇要添加的頁面')
  }
  if (uniquePageIds.length !== args.sourcePageIds.length) {
    throw new PageMergeError('PAGE_MERGE_INVALID_REQUEST', '頁面列表包含重複項')
  }
  if (uniquePageIds.length > MAX_MERGE_PAGE_COUNT) {
    throw new PageMergeError(
      'PAGE_MERGE_PAGE_LIMIT_EXCEEDED',
      `一次最多添加 ${MAX_MERGE_PAGE_COUNT} 頁`
    )
  }
  ...
  if (
    ...
    throw new PageMergeError('PAGE_MERGE_SESSION_BUSY', '當前會話正在生成，暫時不能添加頁面')
  }
  ...
  try {
    ...
    preparedPages = await mapPageMergeConcurrent(
      ...
      async (sourcePage, sourceIndex) => {
        ...
        if (!validation.valid) {
          throw new PageMergeError(
            'PAGE_MERGE_PAGE_COPY_FAILED',
            `頁面“${sourcePage.title}”複製失敗: ${validation.errors.join('; ')}`
          )
        }
    ...
    const persistedPages = await persistManagedPages(ctx, {
      ...
      pages: mergedPages,
      operation: 'addPage',
      prompt: `從${sourceType === 'template' ? '模板' : '會話'}《${sourceTitle}》添加 ${preparedPages.length} 頁`
    })
