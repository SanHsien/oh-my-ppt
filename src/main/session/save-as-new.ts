...
export function registerSessionSaveAsNewHandler(ctx: IpcContext): void {
  ...
  ipcMain.handle('session:saveAsNew', async (_event, payload: unknown) => {
    ...
    if (!sourceSessionId) {
      throw new Error(uiText(locale, '會話 ID 不能爲空', 'Session ID is required.'))
    }
    ...
    if (sourcePages.length === 0) {
      throw new Error(
        uiText(locale, '當前會話沒有可另存的頁面', 'This session has no pages to save as new.')
      )
    }
    ...
    if (!fs.existsSync(sourceProjectDir)) {
      throw new Error(
        uiText(locale, '當前會話目錄不存在，無法另存', 'The current session directory is missing.')
      )
    }
    ...
    const baseTitle =
      typeof sourceSession.title === 'string' && sourceSession.title.trim()
        ...
        : uiText(locale, '未命名會話', 'Untitled session')
    ...
    if (Object.prototype.hasOwnProperty.call(record, 'title') && !requestedTitle) {
      throw new Error(uiText(locale, '會話名稱不能爲空', 'Session title is required.'))
    }
