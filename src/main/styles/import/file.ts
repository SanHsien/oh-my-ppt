...
import { buildStyleImportPrompt } from '../../agent-runtime/prompt'
...
async function prepareStyleSourceFile(
  ...
  if (!ALLOWED_EXTENSIONS.has(ext)) {
    throw new Error(`不支持的文件格式：${ext}，僅支持 ${Array.from(ALLOWED_EXTENSIONS).join(', ')}`)
  }

  const stat = await fs.promises.stat(resolvedPath)
  if (!stat.isFile()) {
    throw new Error(`路徑不是文件：${resolvedPath}`)
  }
  if (stat.size > MAX_FILE_SIZE) {
    throw new Error(
      `文件過大（${(stat.size / 1024 / 1024).toFixed(1)}MB），上限 ${MAX_FILE_SIZE_MB}MB`
    )
  }
