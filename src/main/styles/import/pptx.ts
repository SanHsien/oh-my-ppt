...
import { importPptxToEditableHtml } from '../../io/pptx-import'
import { buildStylePptxImportPrompt } from '../../agent-runtime/prompt'
...
export async function parseStylePptx(args: {
  ...
  if (ext !== '.pptx') throw new Error('不支持的文件格式，僅支持 .pptx')
  const stat = await fs.promises.stat(sourcePath)
  if (!stat.isFile()) throw new Error(`路徑不是文件：${sourcePath}`)
  if (stat.size > MAX_PPTX_SIZE) {
    throw new Error(
      `文件過大（${(stat.size / 1024 / 1024).toFixed(1)}MB），PPTX 上限 ${MAX_PPTX_SIZE_MB}MB`
    )
  }
...
export function selectSamplePagePaths(pagePaths: string[]): string[] {
  ...
  // 等距抽樣中間頁：將中間頁均分爲 N 個桶，每桶取一頁，保證從頭到尾均勻覆蓋，最多 20 頁
...
export function parseStyleImportResponse(response: unknown): StyleParseResult {
  ...
  if (!jsonText) throw new Error('LLM 返回格式異常：未找到 JSON')
  ...
  try {
    ...
    throw new Error(`LLM 返回的 JSON 格式異常：${reason}`)
  }
  ...
  return {
    ...
    category: String(parsed.category || '自定義').trim(),
...
export async function retryFixJson(args: {
  ...
  const result = await model.invoke([
    {
      role: 'user',
      content: `你上次輸出的 JSON 格式有誤，解析報錯：${args.parseError}

請修復 JSON 格式並重新輸出完整的 JSON（用 \`\`\`json ... \`\`\` 包裹）。不要修改內容，只修格式。
