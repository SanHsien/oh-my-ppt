...
function buildFallbackReply(args: {
  ...
  if (args.thinkingUpdated) {
    return '我已經更新了內容方案。你可以繼續補充要求，或確認後進入生成。'
  }
  if (args.contextUpdated) {
    return '我已經記錄了這些需求。你可以繼續補充信息，或讓我開始整理頁面大綱。'
  }
  if (args.stage === 'collect') {
    return '我已收到。請繼續補充目標受衆、使用場景或素材，我會幫你整理成清晰的大綱。'
  }
  return '我已收到，會繼續基於當前方案推進。'
}
...
function summarizeToolCall(toolName: string, rawArgs: unknown): string {
  if (toolName === 'read_file') {
    ...
    if (path) return `正在閱讀資料：${path.replace(/^\/sources\//, '')}`
    return '正在閱讀資料'
  }
  if (toolName === 'grep') {
    const args = typeof rawArgs === 'string' ? safeParseJson(rawArgs) : rawArgs
    const pattern = getNestedField(args, 'pattern') as string | undefined
    if (pattern) return `正在定位相關內容：${String(pattern).slice(0, 24)}`
    return '正在定位相關內容'
  }
  if (toolName === 'update_thinking_document') {
    ...
    if (pages && Array.isArray(pages) && pages.length > 0) {
      ...
      if (titles.length > 0) return `正在整理 ${pages.length} 頁方案：${titles.slice(0, 2).join('、')}${titles.length > 2 ? '…' : ''}`
      return `正在整理 ${pages.length} 頁方案`
    }
    if (topic) return `正在確認主題：${String(topic).slice(0, 24)}`
    return '正在更新方案'
  }
