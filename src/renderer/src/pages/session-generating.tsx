...
const hasTechnicalDetail = (message: string): boolean => {
  ...
  return /Received tool input did not match expected schema|Error invoking tool|ZodError|expected schema|HTML 驗證失敗|HTML 落盤校驗失敗|頁面編輯結果驗證失敗|ERR_FILE_NOT_FOUND|Failed to load URL|文件不存在|at\s+\S+.*:\d+:\d+|<html|<!doctype|data-ppt/i.test(
    compact
  ) || /HTML 末尾|未閉合標籤|開閉標籤數量不一致|內容可能被截斷|<\/?[a-z][\w:-]*(\s|>|\/>)/i.test(compact)
}
...
const friendlyProgressDetail = (detail: string, lang: Lang): string => {
  ...
  if (!compact) return ''
  const pageMatch = compact.match(/(\d+)\/(\d+)\s*(頁|pages?)/i)
  if (pageMatch) {
    return friendlyText(
      lang,
      `已處理 ${pageMatch[1]}/${pageMatch[2]} 頁`,
      `Processed ${pageMatch[1]}/${pageMatch[2]} pages`
    )
  }
  if (/沒有檢測到.*變化|without any detected page changes|no page changes/i.test(compact)) {
    return friendlyText(lang, '剛纔沒有寫入變化，正在換一種方式重試。', 'No changes were written yet; trying another way.')
  }
  if (/HTML 末尾|未閉合標籤|開閉標籤數量不一致|內容可能被截斷|<\/?[a-z][\w:-]*(\s|>|\/>)/i.test(compact)) {
    return friendlyText(
      lang,
      '頁面結構檢查未通過，正在嘗試修復。',
  ...
  if (/schema|工具調用參數|tool call/i.test(compact)) {
    return friendlyText(lang, '工具參數需要修正，正在自動重試。', 'Tool arguments need a quick fix; retrying automatically.')
  }
  if (/校驗|驗證|validat/i.test(compact)) {
    return friendlyText(lang, '頁面結構需要修正，正在自動重試。', 'The page structure needs a fix; retrying automatically.')
  }
  if (/重試|retry/i.test(compact)) {
    return friendlyText(lang, '處理中遇到問題，正在自動重試。', 'Something needs another pass; retrying automatically.')
  }
  if (/準備完成|ready/i.test(compact)) {
    return friendlyText(lang, '準備完成，開始生成頁面。', 'Ready. Starting page generation.')
  }
  const pageTitleMatch = compact.match(/^page-[\w-]+\s*·\s*(.+)$/i)
  if (pageTitleMatch?.[1]) {
    const title = pageTitleMatch[1].trim()
    return friendlyText(lang, `正在處理「${title}」`, `Processing "${title}"`)
  }
  return hasTechnicalDetail(compact) ? '' : compact
}

const isFailureProgress = (label: string | undefined, detail: string): boolean =>
  /失敗|failed|fail|error|錯誤/i.test(`${label || ''} ${detail}`)
...
const friendlyProgressLabel = (label: string | undefined, detail: string, lang: Lang): string => {
  ...
  if (isFailureProgress(label, detail)) {
    return friendlyText(lang, '檢查頁面', 'Checking pages')
  }
  return compactLabel
}
...
const friendlyFailureMessage = (message: string | null | undefined, lang: Lang): string => {
  const compact = compactWhitespace(message || '')
  if (!compact) {
    return friendlyText(lang, '生成沒有完成，請重試。', 'Generation did not finish. Please retry.')
  }
  ...
  if (/schema|tool call|工具調用參數/i.test(compact)) {
    return friendlyText(
      lang,
      '生成工具調用失敗，請重試一次。',
  ...
  if (/校驗|驗證|validat|HTML/i.test(compact)) {
    return friendlyText(
      lang,
      '頁面結果沒有通過檢查，請重試一次。',
  ...
  return hasTechnicalDetail(compact)
    ? friendlyText(lang, '生成沒有完成，請重試。', 'Generation did not finish. Please retry.')
    : compact
}
...
const buildPagePlaceholders = (
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    return {
      ...
      pageNumber,
      title: friendlyText(lang, `第 ${pageNumber} 頁`, `Page ${pageNumber}`),
      status: 'pending'
    }
  })
}
...
export function SessionGeneratingPage({
  ...
  useEffect(() => {
    ...
    const applyChunk = (event: GenerateChunkEvent, options?: { replay?: boolean }): void => {
      ...
      if (event.type === 'llm_status') {
        ...
        const pageMatch = detail.match(/(\d+)\/(\d+)\s*(頁|pages?)/)
        ...
        const isValidationOrError =
          ...
          detail.includes('校驗') || detail.includes('validat') ||
          detail.includes('失敗') || detail.includes('fail') ||
          detail.includes('重試') || detail.includes('retry') ||
          detail.includes('準備完成') || detail.includes('ready')
        const isRetryLabel =
          event.payload.label?.includes('重試') || event.payload.label?.includes('retry')
      ...
      if (event.type === 'page_planned' || event.type === 'page_started' || event.type === 'page_failed') {
        ...
        if (event.type === 'page_failed') {
          appendEvent(
            progressLine({
              label: friendlyText(lang, '頁面生成失敗', 'Page generation failed'),
  ...
  const failedPageSummary =
    failedPreviewLabels.length > 0
      ? friendlyText(
          lang,
          `${failedPreviewLabels.join('、')} 失敗`,
          `${failedPreviewLabels.join(', ')} failed`
        )
      : null
  const failureMessage =
    failedPageSummary ||
    (error && /部分頁面生成失敗|some pages failed|pages failed/i.test(error)
    ...
    <div className="relative flex h-full flex-col overflow-hidden bg-[#edf3e8]">
      ...
      <div className="app-no-drag relative z-10 flex min-h-0 flex-1 flex-col gap-4 px-5 pb-5 pt-4 lg:flex-row">
        <GenerationSidebar
          ...
          logTitle={friendlyText(lang, '生成日誌', 'Generation log')}
