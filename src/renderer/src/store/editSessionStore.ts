...
export const useEditSessionStore = create<EditSessionState>((set, get) => ({
  ...
  save: async () => {
    ...
    try {
      ...
      if (ac > 0) parts.push(`添加 ${ac} 個元素`)
      if (dc > 0) parts.push(`刪除 ${dc} 個元素`)
      if (rc > 0) parts.push(`調整 ${rc} 個元素位置`)
      if (tc > 0) parts.push(`編輯 ${tc} 個元素文字`)
      if (pcount > 0) parts.push(`編輯 ${pcount} 個元素屬性`)
      const prompt = parts.join('、') || '手動調整'
