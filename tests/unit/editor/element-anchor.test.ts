...
describe('patchGenericElementProperties rich text', () => {
  it('updates inline rich text without flattening spans', () => {
    const html = `
      <html><body data-page-id="page">
        <p data-block-id="text"><span style="color:#FB4526" data-block-id="text-1">南歐</span>舊文字</p>
      </body></html>
    `
    ...
    const result = patchGenericElementProperties(
      ...
      {
        html: '<span style="color:#FB4526" data-block-id="text-1">南歐</span>新文字'
      }
    )

    expect(result).toContain(
      '<span style="color:#FB4526" data-block-id="text-1">南歐</span>新文字'
    )
  })
  ...
  it('can update a bare text node by parent selector and child node index', () => {
    const html = `
      <html><body data-page-id="page">
        <p data-block-id="text"><span data-block-id="text-1">南歐</span>舊文字</p>
      </body></html>
    `
    ...
    expect(result).toContain('<span data-block-id="text-1">南歐</span>新文字')
  })
})
...
describe('patchGenericElementProperties formula', () => {
  ...
  it('updates an inline source formula without replacing surrounding text', () => {
    const html = `
      <html><body data-page-id="page">
        <p data-block-id="text">函數 \\(x^2\\) 的導數</p>
      </body></html>
    `
    ...
    expect(result).toContain('函數 \\(x^3\\) 的導數')
  })
  ...
  it('persists display mode changes by rewriting formula delimiters', () => {
    ...
    expect(result).toContain('函數 \\[x^2\\] 的導數')
  ...
  it('does not clear surrounding text when source formula matching fails', () => {
    const html = `
      <html><body data-page-id="page">
        <p data-block-id="text">函數 \\(x^2\\) 與 \\(y^2\\) 的導數</p>
      </body></html>
    `
