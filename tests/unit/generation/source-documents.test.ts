...
describe('resolveSourceDocuments', () => {
  ...
  it('merges newly attached documents with the existing session reference document', async () => {
    ...
    expect(result[0]).toBe('/docs/reference.md')
    expect(result[1]).toMatch(/^\/docs\/extra--[a-f0-9]{16}\.md$/)
    const extraFileName = path.basename(result[1])
    expect(await readFile(path.join(projectDir, 'docs', extraFileName), 'utf8')).toBe('# Extra\n')
  })
