...
describe('size layout skills', () => {
  ...
  it('low-density vertical/poster skills cap content capacity and prescribe overload priority', () => {
    ...
    for (const name of lowDensitySkills) {
      ...
      // Composition starting point is declared.
      expect(source).toContain('Composition starting point')
      // Vertical rhythm guidance with flex distribution
      expect(source).toContain('Vertical rhythm (guidance)')
      expect(source).toContain('flex-1')
      expect(source).toContain('avoid an accidental top-stack or unused lower band')
      // Density resolution priority
      expect(source).toContain('resolve the density in this priority')
      expect(source).toContain('summarize and compress')
