  ...
  const files: Record<string, Uint8Array> = {
    [style + '/style.json']: Buffer.from(
      JSON.stringify(
        {
          style,
          name: { zh: '導入風格', en: 'Imported Style' },
...
describe('style packages', () => {
    ...
    await expect(
      writeStylePackage({
        ...
        json: {
          ...
          name: { zh: '遠程預覽', en: 'Remote Preview' },
  ...
  it('rewrites old installed system packages from bundled styles', async () => {
    ...
    expect(pkg.json.name).toEqual({ zh: '極簡白', en: 'Minimal White' })
  })
  ...
  it('backfills legacy user styles into user packages without preview.html', async () => {
    ...
    expect(stylePackage.json.name.zh).toBe('舊解析風格')
    ...
    expect(warn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        message: expect.stringContaining(
          '跳過用戶風格包回填：style-empty-skill。原因：style_skill 爲空，無法生成 SKILL.md'
        )
      })
    )
    warn.mockRestore()
  ...
  it('does not backfill user packages with invalid style json metadata', async () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        message: expect.stringContaining(
          '跳過用戶風格包回填：style-invalid-json。原因：style.json 無效'
        )
      })
    )
    warn.mockRestore()
  ...
  it('rejects zip packages with files outside the style root', async () => {
    ...
    await expect(importStylePackageZip(zipPath)).rejects.toThrow(/必須只包含/)
  })
})
