...
async function makeStylePackage(): Promise<string> {
  ...
  await writeStylePackage({
    ...
    json: {
      style: 'paper-story',
      name: { zh: '紙上故事', en: 'Paper Story' },
      description: '溫暖、有手作感的敘事風格',
      category: '敘事',
      aliases: [],
      styleCase: '品牌故事',
      version: '1.0.0',
...
describe('generateStylePreview', () => {
  ...
  it('generates in a temporary package and saves a validated preview.html', async () => {
    ...
    const runAgent = vi.fn(async ({ workspaceDir, prompt }) => {
      ...
      await writeFile(
        ...
        '<!doctype html><html><head><style>html,body{width:1600px;height:900px}</style></head><body>把靈感寫在紙上</body></html>',
        'utf8'
      )
    })
    ...
    expect(result).toContain('把靈感寫在紙上')
  ...
  it('does not regenerate a style that already has preview.html', async () => {
    ...
    ).rejects.toThrow('該風格已有預覽，無需重複生成。')
