...
describe('validateHtmlContent declarative animation controls', () => {
  ...
  it('rejects invalid data-anim-sequence values', () => {
    ...
    expect(result.errors.join('\n')).toContain('data-anim-sequence 僅支持 with/after')
  })
  ...
  it('rejects invalid data-anim-stagger values', () => {
    ...
    expect(result.errors.join('\n')).toContain('data-anim-stagger 必須是大於等於 0 的數字毫秒值')
  })
  ...
  it('rejects click-group on non-click animations', () => {
    ...
    expect(result.errors.join('\n')).toContain('data-anim-click-group 只能用於 click 觸發動畫')
  })
  ...
  it('rejects non-contiguous click-group reuse across click steps', () => {
    ...
    expect(result.errors.join('\n')).toContain('data-anim-click-group 必須在 click 動畫的 DOM 順序上連續出現')
  })
  ...
  it('rejects data-anim values outside the public editable contract', () => {
    ...
    expect(result.errors.join('\n')).toContain('data-anim 僅支持當前公開可編輯動畫類型')
    expect(result.errors.join('\n')).toContain('zoom')
    expect(result.errors.join('\n')).toContain('glitch-in')
  })
  ...
  it('rejects non-canonical trigger and from aliases in generated content', () => {
    ...
    expect(result.errors.join('\n')).toContain('data-anim-trigger 僅支持 load/with/after/click')
    expect(result.errors.join('\n')).toContain('on-click')
    expect(result.errors.join('\n')).toContain('data-anim-from 僅支持 left/right/top/bottom/center')
    expect(result.errors.join('\n')).toContain('start')
  })
  ...
  it('allows path only with a non-empty data-anim-path', () => {
    ...
    expect(missing.errors.join('\n')).toContain('data-anim="path" 必須同時提供可解析爲線性位移的 data-anim-path')
  ...
  it('rejects invalid duration and delay values', () => {
    ...
    expect(joined).toContain('data-anim-duration 必須是 100-5000 的數字毫秒值')
    expect(joined).toContain('data-anim-delay 必須是大於等於 0 的數字毫秒值或 stagger(N)')
  })
  ...
  it('rejects runtime-only easing, repeat, and direction knobs in normal editable content', () => {
    ...
    const joined = result.errors.join('\n')
    expect(joined).toContain('data-anim-easing 當前屬於 runtime-only 兼容能力')
    expect(joined).toContain('data-anim-repeat 當前屬於 runtime-only 兼容能力')
    expect(joined).toContain('data-anim-direction 當前屬於 runtime-only 兼容能力')
  })
})
