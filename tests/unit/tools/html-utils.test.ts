...
describe('validateHtmlContent animation validation', () => {
  it('allows declarative data-anim stagger delay', () => {
    ...
    expect(result.errors).not.toContain(
      `檢測到未命名空間的動畫調用（animate/stagger/createTimeline）；修改動畫前請先 ${formatSkillUsageRequirement(DATA_ANIM_SKILL_NAME)}`
    )
  })
...
describe('validateHtmlContent chart height marker validation', () => {
  ...
  it('does not globally apply one marker to multiple charts', () => {
    ...
    expect(result.errors.some((error) => error.includes('圖表高度標記 @ppt-chart-height'))).toBe(
      false
    )
  })
  ...
  it('rejects visible chart height marker text outside HTML comments', () => {
    ...
    expect(result.valid).toBe(false)
    expect(result.errors.some((error) => error.includes('必須寫在 HTML 註釋中'))).toBe(true)
  })
...
describe('validateHtmlContent chart label validation', () => {
  it('rejects Chart.js labels that contain HTML markup', () => {
    ...
    expect(result.valid).toBe(false)
    expect(result.errors.some((error) => error.includes('圖表 labels 包含 HTML 標籤'))).toBe(true)
  })
  ...
  it('allows Chart.js string-array labels for multi-line categories', () => {
        ...
        <script>
          document.addEventListener('DOMContentLoaded', function() {
            PPT.createChart(document.getElementById('chart'), {
              type: 'bar',
              data: {
                labels: [['AI調校師', '約80→1,400'], ['中割/補間', '9,300→5,600']],
                datasets: [{ label: '2026人數', data: [1400, 5600] }]
              },
              options: { responsive: true, maintainAspectRatio: false }
            })
          })
    ...
    expect(result.errors.some((error) => error.includes('圖表 labels 包含 HTML 標籤'))).toBe(false)
  })
})
...
describe('validatePersistedPageHtml chart validation', () => {
  ...
  it('rejects persisted pages with malformed inline chart scripts', () => {
    ...
    expect(result.valid).toBe(false)
    expect(result.errors.some((error) => error.includes('內聯 script 語法錯誤'))).toBe(true)
  })
                ...
                <script>
                  document.addEventListener('DOMContentLoaded', function() {
                    PPT.createChart(document.getElementById('chart'), {
                      type: 'bar',
                      data: {
                        labels: ['中割/補間<br><span>9,300→5,600</span>'],
                        datasets: [{ data: [-39.8] }]
                      },
                      options: { responsive: true, maintainAspectRatio: false }
                    })
