...
def check_upstream_website(baseline: dict) -> dict | None:
    website_url = baseline.get("website")
    if not website_url:
        return None
    version_url = f"{website_url.rstrip('/')}/version.json"
    try:
        req = urllib.request.Request(
            version_url,
            headers={"User-Agent": "oh-my-ppt-upstream-checker/1.0"},
        )
        with urllib.request.urlopen(req, timeout=10) as response:
            if response.status != 200:
                return {"error": f"HTTP {response.status}", "url": website_url}
            data = json.loads(response.read().decode("utf-8"))
            return {
                "url": website_url,
                "current_version": str(data.get("version", "")).strip(),
                "reviewed_version": str(baseline.get("reviewed_website_version", "")).strip(),
                "download_home": str(data.get("downloadhome", "")).strip(),
            }
    except Exception as exc:
        return {"error": str(exc), "url": website_url}
...
def render_markdown(
    ...
    def with_tickets(body: list[str]) -> str:
        ...
        if website_info:
            body.extend(["## Upstream documentation website", ""])
            if "error" in website_info:
                body.extend([
                    f"- Website: `{website_info.get('url', 'unknown')}`",
                    f"- Status check warning: {website_info['error']}",
                    "",
                ])
            else:
                cur_v = website_info.get("current_version", "")
                rev_v = website_info.get("reviewed_version", "")
                body.extend([
                    f"- Website: `{website_info['url']}`",
                    f"- Current online version: `{cur_v}` (Reviewed through: `{rev_v}`)",
                ])
                if cur_v and rev_v and cur_v != rev_v:
                    body.extend([
                        "",
                        f"⚠️ Upstream website version changed from `{rev_v}` to `{cur_v}`! Requires documentation review.",
                        "",
                    ])
                else:
                    body.extend(["- Documentation website status: Up to date.", ""])
        return "\n".join(body)
...
def main() -> int:
    ...
    try:
        ...
        website_info = check_upstream_website(baseline)
    ...
    report = render_markdown(baseline, commits, prs, issues, website_info, error)
    ...
    if unavailable:
        ...
        return 2
    website_changed = bool(website_info and not website_info.get('error') and website_info.get('current_version') != website_info.get('reviewed_version'))
    if args.strict and (commits or prs or issues or website_changed):
        return 1
    return 0
